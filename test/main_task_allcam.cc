...
std::vector<int> topmode ={0};
std::vector<int> downmode ={20, 21, 22, 23};
...
mode.clear();
mode.push_back(topmode);
mode.push_back(downmode);
    ...
    std::vector<int> topmode111;



    for(int frameid = 0; frameid < img_paths.size(); ++frameid)
    {
        if(frameid == img_paths.size()-2)
        {
            frameid = 0;
        }
        for (int cam_id = 0; cam_id < mode.size(); cam_id++)
        { 
            if(cam_id ==0)
            {
                topmode111 = topmode;
            ...
            for (size_t k = 0; k < topmode111.size(); k++)
            {
                ...
                std::cout << "image_path = " << imgs_RGB_path.c_str() << std::endl;
    ...
    /********   读取 RGB 数据 ***********************************************/
                ...
                struct_img[0].img_type   = ECOAINTEFACE_IMG_SRC_TYPE_E(ECOAINTEFACE_IMG_SRC_TYPE_RGBD1 + cam_id);
    ...
    /********   读取 depth 数据 ***********************************************/
                ...
                struct_img[1].img_type   = ECOAINTEFACE_IMG_SRC_TYPE_E(ECOAINTEFACE_IMG_SRC_TYPE_RGBD1 + cam_id);
    ...
    /********   读取 ir 数据 ***********************************************/
                ...
                struct_img[2].img_type   = ECOAINTEFACE_IMG_SRC_TYPE_E(ECOAINTEFACE_IMG_SRC_TYPE_RGBD1 + cam_id);
            ...
            ofs << 0 << " " << strDest[0] << " " << modle_id << " " <<  strDest[2] << " " << strDest[1] << " " << strDest[3] << " ";
                ...
                struct_ctr.model_id = topmode111[k];
                ...
                std::cout << "image[ " << k << " ]_process time = " << duration.count() << "ms" << std::endl;
                    ...
                    if (NULL != ecogroundobjects)
                    {
                        ...
                        for (int outnum = 0; outnum < ecogroundobjects->ngroundobjectnum; outnum++)
                        {
                            ...
                            if(ecoinstanceobject->maskdata[2 * outnum + 0].bistrue || ecoinstanceobject->maskdata[2 * outnum + 1].bistrue)
                            {
                                ...
                                ofs << points3.y << " " << points3.x << " " << points4.y << " " << points4.x << " ";

    #endif
                                sprintf(text, "(%d,%d)", (int)points3.y, (int)points3.x);
                                putText(img_RGB, text, cv::Point(roi.x, roi.y + roi.height),
                                            cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 5);
                                sprintf(text, "(%d,%d)", (int)points4.y, (int)points4.x);
                        ...
                        if (ecoinstanceobject->maskdata.size() > 0)
                        {
                            for (size_t kk = 0; kk < ecoinstanceobject->maskdata.size(); kk++)
                            {
                                if (ecoinstanceobject->maskdata[kk].bistrue)
                                {
                                    ...
                                    cv::Point2f points3 = ecoinstanceobject->maskdata[2 * kk + 0].mappos;
                                    cv::Point2f points4 = ecoinstanceobject->maskdata[2 * kk + 1].mappos;
                                    ofs << points3.y << " " << points3.x << " " << points4.y << " " << points3.x << " ";
    #endif
