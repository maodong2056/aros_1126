...
const int anchor[3][6] = {{3, 4, 5, 8, 23, 24},
                          {12, 16, 81, 90, 232, 172},
                          {111, 64, 327, 300, 438, 286}};
...
static int process_i8(int8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, std::vector<float> &threshold,
                      int32_t zp, float scale)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;

    float min_threshold = *std::min_element(threshold.begin(), threshold.end());
    int8_t thres_i8 = qnt_f32_to_affine(unsigmoid(min_threshold), zp, scale);
    for (int i = threshold.size(); i < OBJ_CLASS_NUM_OBSTACLE; i++) {
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                int8_t box_confidence = input[(PROP_BOX_SIZE_DRYSTAIN * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_i8)
                {
                    int offset = (PROP_BOX_SIZE_DRYSTAIN * a) * grid_len + i * grid_w + j;
                    ...
                    int maxClassId = 0;
                    for (int k = 1; k < OBJ_CLASS_NUM_DRYSTAIN; ++k)
                    {
                    ...
                    float limit_score = sigmoid(deqnt_affine_to_f32(maxClassProbs, zp, scale)) * sigmoid(deqnt_affine_to_f32(box_confidence, zp, scale));
                    if (limit_score > threshold[maxClassId])
                    {
                        ...
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_confidence = input[(PROP_BOX_SIZE_DRYSTAIN * a + 4) * grid_len + i * grid_w + j];
...
int drystain_post_process_zero_copy(sweeper_ai::EcoRknnModelParams& modelparams, rknn_tensor_mem **_outputs, scale_t *scale_wh, std::vector<float>& conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals, int want_float)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        BOX_PROP prop;
        outputresult.box.left   = (int)((clamp(x1, 0, model_in_w) / scale_wh->scale_w)*2.5);
        outputresult.box.top    = (int)((clamp(y1, 0, model_in_h) / scale_wh->scale_h + 208)*2.5);
        outputresult.box.right  = (int)((clamp(x2, 0, model_in_w) / scale_wh->scale_w)*2.5);
        outputresult.box.bottom = (int)((clamp(y2, 0, model_in_h) / scale_wh->scale_h + 208)*2.5);
