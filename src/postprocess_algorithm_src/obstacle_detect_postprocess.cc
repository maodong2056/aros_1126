                    ...
                    std::vector<float> &threshold)
{
    ...
    int8_t score_thres_i8_array[OBJ_CLASS_NUM_OBSTACLE];
    for (int i = 0; i < threshold.size() && i < OBJ_CLASS_NUM_OBSTACLE; i++) {
        score_thres_i8_array[i] = qnt_f32_to_affine(unsigmoid(threshold[i]), score_zp, score_scale);
    }

    for (int i = threshold.size(); i < OBJ_CLASS_NUM_OBSTACLE; i++) {
        score_thres_i8_array[i] = score_thres_i8_array[0];
    }

    int8_t score_sum_thres_i8 = qnt_f32_to_affine(unsigmoid(min_threshold), score_sum_zp, score_sum_scale);
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            for (int c= 0; c< OBJ_CLASS_NUM_OBSTACLE; c++){
                if ((score_tensor[offset] > score_thres_i8_array[c]) && (score_tensor[offset] > max_score))
                {
            ...
            // compute box
            if (max_class_id >= 0 && max_score > score_thres_i8_array[max_class_id]){
                offset = i* grid_w + j;
                float box[4];
...
// 零拷贝
// 修改rknn_output *_outputs为rknn_tensor_mem **_outputs
// _outputs[].buf改为_outputs[]->virt_addr
int obstacle_yolov8_post_process_zero_copy(sweeper_ai::EcoRknnModelParams& modelparams, rknn_tensor_mem **_outputs, scale_t *scale_wh, std::vector<float>& conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals, int want_float)
{
    ...
    int dfl_len    = modelparams.nmodeloutputchannel_[3] /4;
    int output_per_branch = modelparams.io_num.n_output / 3;

    int branch_order[6] = {3,4,5,0,1,2};
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        int box_idx = branch_order[i];
        int score_idx = branch_order[i+ 3] ;
