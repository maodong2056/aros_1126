                      ...
                      int32_t zp, float scale)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                int8_t box_confidence = input[(PROP_BOX_SIZE_PM * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_i8)
                {
                    int offset = (PROP_BOX_SIZE_PM * a) * grid_len + i * grid_w + j;
                    ...
                    int maxClassId = 0;
                    for (int k = 1; k < OBJ_CLASS_NUM_PM; ++k)
                    {
                        ...
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_confidence = input[(PROP_BOX_SIZE_PM * a + 4) * grid_len + i * grid_w + j];
...
int pm_post_process(sweeper_ai::EcoRknnModelParams& modelparams, rknn_output *_outputs, scale_t *scale_wh, float conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals)
{
...
int pm_post_process_zero_copy(sweeper_ai::EcoRknnModelParams& modelparams, rknn_tensor_mem **_outputs, scale_t *scale_wh, float conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals, int want_float)
{
