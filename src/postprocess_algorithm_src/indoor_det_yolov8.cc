    ...
    float nms_threshold, std::vector<detect_result_t> &proposals)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            validCount += process_i8((int8_t *)outputs[box_idx].buf, modelparams.out_zps[box_idx], modelparams.out_scales[box_idx],
                                     (int8_t *)outputs[score_idx].buf, modelparams.out_zps[score_idx], modelparams.out_scales[score_idx],
