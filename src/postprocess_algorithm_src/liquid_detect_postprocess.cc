                    ...
                    float threshold)
{
    ...
    int8_t score_thres_i8 = qnt_f32_to_affine(unsigmoid(threshold), score_zp, score_scale);
    int8_t score_sum_thres_i8 = qnt_f32_to_affine(unsigmoid(threshold), score_sum_zp, score_sum_scale);
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            ...
            for (int c= 0; c< OBJ_CLASS_NUM_LIQUID; c++){
...
int liquid_post_process_zero_copy(sweeper_ai::EcoRknnModelParams& modelparams, rknn_tensor_mem **_outputs, scale_t *scale_wh, float conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals, int want_float)
{
    ...
    int dfl_len    = modelparams.nmodeloutputchannel_[1] /4;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        int box_idx = i*output_per_branch + 1; //i*output_per_branch;
        int score_idx = i*output_per_branch; //i*output_per_branch + 1;
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        outputresult.box.top    = (int)((clamp(y1, 0, model_in_h) / scale_wh->scale_h + 520));
        outputresult.box.right  = (int)((clamp(x2, 0, model_in_w) / scale_wh->scale_w));
        outputresult.box.bottom = (int)((clamp(y2, 0, model_in_h) / scale_wh->scale_h + 520));
