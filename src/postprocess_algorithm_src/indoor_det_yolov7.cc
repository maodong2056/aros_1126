                      ...
                      int32_t zp, float scale)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                int8_t box_confidence = input[(PROP_BOX_SIZE_INDOOR_YOLOV7 * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_i8)
                {
                    int offset = (PROP_BOX_SIZE_INDOOR_YOLOV7 * a) * grid_len + i * grid_w + j;
                    ...
                    int maxClassId = 0;
                    for (int k = 1; k < OBJ_CLASS_NUM_INDOOR_YOLOV7; ++k)
                    {
                        ...
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_confidence = input[(PROP_BOX_SIZE_INDOOR_YOLOV7 * a + 4) * grid_len + i * grid_w + j];
...
float nms_threshold, std::vector<detect_result_t> &proposals)
{
    ...
    ////这句就是为了防止classId这个vector偶尔会报错--先临时放在这
    // classId.reserve(500);
    int validCount = 0;
    int stride = 0;
    int grid_h = 0;
    int grid_w = 0;
    int model_in_w = (modelparams.nmodelinputweith_)[0];  ////这个参数传进去了，但好像在函数中没用--目前将其认定为模型输入的宽
    int model_in_h = (modelparams.nmodelinputheight_)[0]; ////这个参数传进去了，但好像在函数中没用--目前将其认定为模型输入的高
