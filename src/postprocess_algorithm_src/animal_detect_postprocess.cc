...
#define ANIMAL_OBJ_CLASS_NUM 2 //80
#define ANIMAL_PROP_BOX_SIZE (5 + ANIMAL_OBJ_CLASS_NUM)
namespace sweeper_ai
{
// const int anchor[3][6] = {{10, 13, 16, 30, 33, 23},
//                           {30, 61, 62, 45, 59, 119},
//                           {116, 90, 156, 198, 373, 326}};
const int anchor[3][6] = {{33,28, 38,48, 63,39},  // P3/8
                            {61,66, 104,66, 80,104},  // P4/16
                            {137,91, 122,143, 208,164}};
                      ...
                      int32_t zp, float scale)
{
    ...
    float arc_thresh = log(threshold/(1-threshold));
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                float box_confidence = deqnt_affine_to_f32(input[(ANIMAL_PROP_BOX_SIZE * a + 4) * grid_len + i * grid_w + j], zp, scale);
                if (box_confidence >= arc_thresh)
                {
                    int offset = (ANIMAL_PROP_BOX_SIZE * a) * grid_len + i * grid_w + j;
                    int8_t *in_ptr = input + offset;

                    int8_t maxClassProbs = in_ptr[5 * grid_len];
                    int maxClassId = 0;
                    for (int k = 1; k < ANIMAL_OBJ_CLASS_NUM; ++k)
                    {
                    ...
                    float confidence = sigmoid(box_confidence) * sigmoid(deqnt_affine_to_f32(maxClassProbs, zp, scale));
                        ...
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_confidence = input[(ANIMAL_PROP_BOX_SIZE * a + 4) * grid_len + i * grid_w + j];
...
// int animal_post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        result_a.box.left          = (int)(clamp(x1, 0, model_in_w) / scale_wh->scale_w);
        result_a.box.top           = (int)(clamp(y1, 0, model_in_h) / scale_wh->scale_h);
        result_a.box.right         = (int)(clamp(x2, 0, model_in_w) / scale_wh->scale_w);
        result_a.box.bottom        = (int)(clamp(y2, 0, model_in_h) / scale_wh->scale_h);
