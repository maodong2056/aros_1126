...
const float anchor[3][6] = {{11.7, 10.0, 23.6, 20.6, 36.2, 38.8},
                          {62.8, 49.7, 52.2, 83.4, 93.9, 80.8},
                          {91.1, 138.2, 142.5, 111.8, 187.8, 188.3}};
...
static int process_i8(int8_t *input, float *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      ...
                      int32_t zp, float scale)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                int8_t box_confidence = input[(PROP_BOX_SIZE_LINT * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_i8)
                {
                    int offset = (PROP_BOX_SIZE_LINT * a) * grid_len + i * grid_w + j;
                    ...
                    int maxClassId = 0;
                    for (int k = 1; k < OBJ_CLASS_NUM_LINT; ++k)
                    {
                        ...
                        std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_confidence = input[(PROP_BOX_SIZE_LINT * a + 4) * grid_len + i * grid_w + j];
...
int lint_post_process_zero_copy(sweeper_ai::EcoRknnModelParams& modelparams, rknn_tensor_mem **_outputs, scale_t *scale_wh, float conf_threshold, float nms_threshold, std::vector<detect_result_t>& proposals, int want_float)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (!want_float)
        {
            validCount += process_i8((int8_t *)_outputs[i]->virt_addr, (float *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
